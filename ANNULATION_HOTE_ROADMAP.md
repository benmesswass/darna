...
| AH3 | Suggestions de relogement : `getRebookingSuggestions` (ville, capacité, prix, disponibilité réelle) | P0 | ✅ | `src/lib/listings.ts`. PR AH3/AH4 (branche `claude/host-cancellation-ah3`). |
| AH4 | Réduction ponctuelle liée à la suggestion (token signé, usage unique) | P1 | ✅ | `src/lib/rebooking-discount.ts`, branché sur `createBookingAction`/`quoteBookingAction`. PR AH3/AH4 (branche `claude/host-cancellation-ah3`). |
...
| AH6 | UI voyageur : notification d'annulation + suggestions + réduction | P0 | 🟡 | La notification (AH1) pointe désormais directement vers la meilleure suggestion (AH3) avec le token de réduction (AH4) déjà appliqué — choix documenté : lien direct vers 1 alternative plutôt qu'une page listant plusieurs suggestions. Reste ouvert : pas de page/section dédiée listant plusieurs alternatives si aucune n'est trouvée en ville exacte. |
