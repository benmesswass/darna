...
| MI5 | Apport d'affaires financement : modèle `FinancingLead` (simulation crédit demandée depuis une fiche immo) + export/dashboard admin pour un partenaire bancaire, commission au succès facturée à la banque | P2 | 🔧 | **Capture du lead livrée en amont (PR #197)** — reste ⏸️ bloqué pour la monétisation réelle (commission facturée à la banque), qui dépend d'une convention externe non signée, hors scope de ce dépôt. Modèle `FinancingLead` (migration `20260724153117_add_financing_lead`, même patron que `ContactRequest`) : name/email/phone/`desiredAmount` (optionnel, TND)/message (optionnel), lié à la `Property` (VENTE **uniquement** — un financement est un prêt à l'achat, n'a pas de sens pour une location) et au `sender` si connecté. `submitFinancingLeadAction` (`src/actions/financing-lead.ts`) : zod, rate-limité (`assertRateLimit("financing-lead")`), vérifie annonce VENTE + `ACTIVE`. **Aucune simulation de mensualité calculée par Darna** — contrairement à `/combien-gagner` (§G1), aucune donnée réelle (taux, critères bancaires) ne permettrait de la fonder honnêtement ; le formulaire l'indique explicitement (disclaimer). `FinancingLeadSection` affichée sur `ImmoDetail.tsx` seulement pour `type === "VENTE"`, après `ImmoContactSection`. Dashboard admin `/dashboard/admin/financement` (lecture seule, ADMIN uniquement) + export CSV (`/api/admin/financing-leads/export`, neutralise l'injection de formule CSV/OWASP sur les champs saisie libre). i18n fr/en/ar. Tests : `tests/financing-lead-action.test.ts` (9 tests). Vérifié en Playwright (Postgres local, compte `admin@darna.tn`) : section absente sur une annonce LOCATION, présente et fonctionnelle sur une annonce VENTE, lead visible dans le dashboard admin, export CSV téléchargé avec les bonnes données. |
...
6. 🔧 MI5 — apport d'affaires financement. Capture du lead ✅ (PR #197) ; reste ⏸️ bloqué pour la monétisation (commission bancaire).
...
> MI1-MI3). MI5 : capture du lead faite (PR #197) ; rien de plus à préparer
> côté code tant qu'aucun partenariat bancaire n'est signé.
