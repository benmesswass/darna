...
| MI6 | QA/sécurité : tests idempotence paiement boost, IDOR abonnement (un hôte ne peut pas payer/voir l'abonnement d'un autre), non-bypass des limites d'annonces actives ; mise à jour `QA_ROADMAP.md` | P0 | 🔧 | Transverse — à livrer avec chaque phase concernée, pas uniquement à la fin (même règle que PSP7). MI0 (`QA_ROADMAP.md` §6.2), MI2 (§6.3) et MI3 (§6.4, y compris `tests/verification-credit-webhook.test.ts` — dernier gap fermé, la route avait la garde de signature mais aucun test HTTP dédié) livrées. Reste à couvrir au fur et à mesure de MI4-MI5. |
...
**Transverse :**
7. 🔧 MI6 — QA/sécurité, à chaque phase. Portions MI0 (`QA_ROADMAP.md` §6.2), MI2 (`QA_ROADMAP.md` §6.3) et MI3 (`QA_ROADMAP.md` §6.4, dont le test webhook dédié `verification-credit-webhook.test.ts`) livrées ; reste à couvrir au fur et à mesure de MI4-MI5.
