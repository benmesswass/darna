...
**Chaînage automatique des roadmaps de chantier (règle absolue).** Quand une roadmap de chantier a **toutes ses phases `✅`** ET porte un **pointeur de continuation** en fin de fichier (bloc `➡️ CONTINUATION : <FICHIER>`), « suivant » / « enchaîne » sur ce chantier **bascule AUTOMATIQUEMENT** sur la roadmap de continuation pointée et propose sa **première tâche non cochée** (`P0` d'abord) — sans redemander à Wassim quel fichier ouvrir. Suivre la chaîne de pointeurs de proche en proche si la roadmap de continuation est elle-même terminée. **Cas actif :** `ANNULATION_HOTE_ROADMAP.md` (AH0→AH7 tous `✅`) ➡️ `ANNULATION_HOTE_CORRECTIFS_ROADMAP.md` — donc « suivant » / « enchaîne » sur l'annulation hôte attaque directement **AHC1 (P0)**, puis AHC2, etc.
