...
## Roadmap (règle absolue)

**`ROADMAP.md` est le fichier UNIQUE de pilotage du projet** (créé le 2026-07-28). Il remplace et consolide toutes les roadmaps antérieures (`LANCEMENT`, `PRIORITES`, `GROWTH`, `CROISSANCE`, `QA`, `PAIEMENT_SUR_PLACE`, `MONETISATION_IMMO`, `TEST_AUTOMATION`, `INFRA`, `TODO-BETA`, `TODO-PRODUCTION`, `AUDIT_V1/V2`), supprimées après récolte exhaustive de leurs items ouverts — l'historique git les conserve. **Il n'existe plus qu'une seule roadmap : ne jamais en recréer une autre, ne jamais rouvrir un fichier supprimé.**

**Dès que Wassim demande de continuer — « quelle est la prochaine étape / étape suivante / quoi coder ensuite / suivant / enchaîne / on enchaîne »** (toute formulation équivalente, dans n'importe quelle session y compris une toute nouvelle), **TOUJOURS ouvrir `ROADMAP.md` en premier** et prendre **la première tâche non cochée en partant du haut** (phase 1 → phase 8, dans l'ordre du fichier) — jamais une tâche choisie plus loin parce qu'elle paraît plus simple, jamais une tâche improvisée hors de ce fichier. Une tâche bloquée sur Wassim (🧑/⛔) se saute en le signalant, sans jamais rester bloqué. Les règles complètes de session sont dans `ROADMAP.md` §0, qui fait autorité.

**Le gel des features de croissance (phase 7 de `ROADMAP.md`) s'applique tant que ses trois conditions de dégel ne sont pas TOUTES vraies** (production en ligne + ≥ 20 annonces réelles vérifiées + ≥ 5 réservations réelles). Aucune tâche de la phase 7 ne doit être lancée avant, y compris en mode orchestration autonome.
...
4. **Surveiller la CI** : pyramide en 3 niveaux (`ROADMAP.md` §P1.2, `.github/workflows/ci.yml` + `nightly.yml`). Niveau 1 `fast` (lint + typecheck + tests sans couverture) tourne automatiquement sur chaque push/màj de PR (hors PR *.md/docs, ignorées). Avant de proposer un merge, déclencher le niveau 2 — coverage + build + audit + e2e + api — en posant le label `ready-to-merge` sur la PR (ou via `workflow_dispatch`). Le niveau 3 (`nightly.yml` : semgrep + ZAP + k6) tourne chaque nuit sur `main`, hors chemin de merge d'une PR. Gate indépendant de la pyramide (`migration-gate`, P5.5 `ROADMAP.md`) : bloque toute PR modifiant une migration destructive (DROP/ALTER de colonne, TRUNCATE) tant que le label `migration-approved` n'est pas posé — tourne sur tout événement de PR, y compris `labeled`. « CI verte » = `fast`, `gitleaks` et `migration-gate` ET les jobs du niveau 2 tous verts sur le commit de tête.
...
1. **Périmètre strict** : uniquement les tâches déjà listées dans `ROADMAP.md`, dans l'ordre des phases, jamais une tâche inventée ou hors roadmap. **Sauter silencieusement** toute tâche marquée « ⛔ Bloqué sur toi » (attend un arbitrage produit de Wassim) — pas prête à exécuter, jamais un candidat au merge auto.
...
3. **Merge automatique autorisé** dès que (2) est rempli — squash, sans attendre le mot « merge » de Wassim pour cette tâche précise. `ROADMAP.md` mis à jour (✅ + PR) **dans la même PR**, comme déjà exigé par la règle générale.
...
5. **Un chantier à la fois** : jamais paralléliser deux tâches de `ROADMAP.md`. Passer au suivant non coché dans l'ordre de priorité dès le précédent mergé.
...
- **Scheduler (amendement du 2026-07-27, `ROADMAP.md` §2 ; véhicule corrigé le 2026-07-30, `docs/INFRASTRUCTURE.md` §3 piège n°1)** : **zéro cron pour l'ÉTAT** (lazy-expiry conservée partout — `expiresAt`, `featuredUntil`, `promoUntil`, crédits FIFO, retard de facture dérivé — NE JAMAIS réécrire ces mécanismes en jobs), **un scheduler pour les ACTIONS SORTANTES** (e-mail/notification, comparaison avec un système externe type Konnect, purge). Véhicule : `/api/jobs/tick` (toutes les 15 min), protégé par `CRON_SECRET` — appelé par un **cron externe** (cron-job.org), **pas** le Cron natif Vercel (`vercel.json` supprimé : bloquait tout déploiement sur le plan Hobby, pas seulement en retard). Chaque job = fonction pure idempotente enregistrée dans `src/lib/jobs/runner.ts`.
...
## Paiement Konnect (frais de service — commission-only, §L5.1)

Konnect ne règle plus jamais que les **frais de service Darna** (`depositAmount`/`amountPaid`, ~10 % du loyer) — jamais le loyer, qui se règle en espèces à l'hôte, à l'arrivée. Le paiement intégral en ligne (vrai séquestre, code `escrow` du schéma conservé inerte) est reporté à la **V2**, conditionnée à l'avis juridique W4.
