...
| G8 | Fraîcheur de vérification visible dans les **résultats de recherche** (pas seulement la fiche détail) | P2 | ❌ | CTR selon fraîcheur de vérification (< 30 j vs plus ancien) | `VerifiedBadge` affiche déjà vérificateur + date en mode non-`small` (fiche détail) — `PropertyCard.tsx` l'appelle en mode `small` sans date (`Badges.tsx:79`). Wiring uniquement : passer `verifiedAt`/`verifierName` au badge des cartes de recherche (tooltip, pas d'encombrement visuel). Donnée déjà en base (`Property.verifiedAt`), aucune migration. |
...
| G10 | Mur de la confiance en direct — objectif 500 annonces vérifiées rendu public | P2 | ❌ | Clics compteur → recherche/inscription hôte | `HomeHero` affiche déjà `verifiedCount` brut (`src/app/page.tsx:31,46`) — ajoute la mise en contexte (progression vers l'objectif north-star 500, `.agents/product-marketing.md`) + un fil « dernières vérifications » (ville + date, zéro donnée personnelle) qui n'existe pas aujourd'hui. Rigoureusement honnête, aucun chiffre gonflé. |
