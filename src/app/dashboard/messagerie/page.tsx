...
export default async function MessageriePage() {
  ...
  return (
    <div>
      <h1 className="text-2xl font-bold text-darna">{fr.messages.hubTitre}</h1>
      <p className="mt-1 text-sm text-ink/60">{fr.messages.hubSousTitre}</p>

      {conversations.length === 0 ? (
        <div className="mt-6 rounded-2xl bg-white px-4 py-12 text-center ring-1 ring-darna/10">
          <MailIcon width={28} height={28} className="mx-auto text-darna/40" />
          <p className="mt-3 text-sm text-ink/55">{fr.messages.hubVide}</p>
        </div>
      ) : (
          ...
          {conversations.map((c) => (
            <li key={c.bookingId}>
              <Link
                ...
                className="flex items-center gap-3 rounded-2xl bg-white px-4 py-3.5 ring-1 ring-darna/10 transition hover:ring-darna/30"
              >
                <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-cream text-darna">
                  <MailIcon width={18} height={18} />
                </span>
                <span className="min-w-0 flex-1">
                  <span className="flex items-center justify-between gap-2">
                    <span className="truncate font-semibold text-darna">{c.otherName}</span>
                  ...
                  <span
                    className={`mt-1 block truncate text-sm ${
                      c.unread > 0 ? "font-semibold text-ink" : "text-ink/60"
                    }`}
                  >
