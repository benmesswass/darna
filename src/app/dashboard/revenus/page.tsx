    ...
    <div>
      ...
      {bookings.length === 0 ? (
        <div className="mt-6 rounded-3xl bg-white p-10 text-center ring-1 ring-darna/10">
          <p className="text-lg font-semibold text-darna">{fr.dashboard.revenusAucun}</p>
          <p className="mt-1 text-sm text-ink/60">{fr.dashboard.revenusAucunCta}</p>
        </div>
      ) : (
          ...
          {bookings.map((b) => {
            ...
            return (
              ...
              >
                ...
                <p className="shrink-0 text-lg font-bold text-darna">
  ...
  return (
    <div
      className={`rounded-3xl p-5 ring-1 ${
        emphasis ? "bg-darna ring-darna" : "bg-white ring-darna/10"
      }`}
    >
      <p className={`text-xs font-semibold ${emphasis ? "text-white/80" : "text-ink/55"}`}>
        {label}
      </p>
      <p className={`mt-1.5 text-2xl font-bold ${emphasis ? "text-white" : "text-darna"}`}>
        {value}
      </p>
    </div>
  );
