...
/**
 * Lance le paiement Konnect de l'abonnement agence puis redirige vers la
 * passerelle (`payUrl`). Miroir exact de FeaturedPayButton/PayHostInvoiceButton.
 *
 ...
 */
export function SubscriptionPayButton({ plan, label }: { plan: string; label: string }) {
  const fr = useT();
  const [state, action, pending] = useActionState<SubscriptionPaymentState, FormData>(
    startSubscriptionPaymentAction,
    undefined
  );
