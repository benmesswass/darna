...
| AHC7 | **Avis/signal automatique « annulé par l'hôte »** (dissuasif réputationnel, emprunt Airbnb, aligné north-star) | P2 | ✅ | Décisions produit du 2026-07-08 : forme = entrée SYSTÈME insérée dans le flux d'avis de l'annonce (pas un badge compteur), dès la 1re annulation, fenêtre glissante 90j (`HOST_CANCELLATION_SIGNAL_DAYS`, `src/lib/config.ts`). `getHostCancellationSignals` (`src/lib/listings.ts`) — id + date uniquement, jamais le voyageur (vie privée). `ReviewsList`/`ReviewsSection` (`src/components/property/`) fusionnent l'entrée dans le flux chronologique, JAMAIS dans la moyenne/histogramme/sous-notes ni sous un filtre/tri par note (pas de note). i18n fr/en/ar. Tests `tests/rebooking-suggestions.test.ts` (requête) + `tests/components/reviews-list.test.tsx` (6 tests UI). Vérifié en direct (fiche annonce publique, sans connexion). |
