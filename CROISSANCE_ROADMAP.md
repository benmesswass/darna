...
| CR1 | Parcours voyageur : page « Mes crédits », bouton « Parrainer » (réutilise `ShareButton.tsx`, F8), application du solde au checkout | P0 | ✅ | PR #188. Filleul : **+15 TND** (`REFERRAL_SIGNUP_BONUS_TND`) immédiat à l'inscription via lien/code (`?ref=CODE`, `src/actions/auth.ts`) — jamais bloquant si l'émission échoue. Page `/dashboard/credits` : solde, code/lien de parrainage, bouton « Parrainer » (`ShareButton` étendu avec `message`/`label`/`context`), historique. Checkout (`src/actions/bookings.ts`) : case à cocher opt-in, `computeCreditApplication` (pure, `src/lib/credits.ts`) plafonne à `CREDIT_CHECKOUT_CAP_RATE` (100 % depuis §L5.1, commission-only — périmé : plafonnait à 30 % du `totalPrice` avant §L5.1) des frais Darna RESTANTS (`totalPrice - subtotal`) — jamais au-delà, le prix hôte (`subtotal`) ne bouge jamais, cumulable avec `RebookingDiscount`. Dépense atomiquement couplée à la transaction de réservation (`spendCredit(…, tx)`). Tests : `tests/credits.test.ts`, `tests/auth-referral.test.ts`, `tests/booking-credit-application.test.ts`, `tests/components/share-button.test.tsx`. Vérifié en direct (inscription avec code → +15 TND visible → case à cocher au checkout → total réduit en direct).|
| CR2 | Parrainage hôte, débloqué à la 1ère annonce vérifiée `ACTIVE` + 1ère résa confirmée de l'hôte parrainé | P1 | 🔧 | PR #192 (partiel, 2026-07-24) : **détection + crédit du parrain faits** — `settleHostReferralMilestones` (`src/lib/credits.ts`), lazy à la lecture de `/dashboard/credits`, jalon = annonce vérifiée `ACTIVE` + résa **`TERMINEE`** (pas `CONFIRMEE`), motif `PARRAINAGE_FILLEUL_TERMINE`, idempotent, plafonné `HOST_REFERRAL_YEARLY_CAP`=5/an. Vérifié en direct sur données réelles. **Reste à faire** : rendre ce crédit dépensable sur vérification Wakil / boost à la une / abonnement (texte original ci-dessous) — touche 3 flux Konnect déjà en prod, chacun avec un cas de couverture à 100 % par crédit qui doit contourner Konnect (montant nul non supporté) + UI de paiement correspondante ; volontairement traité comme un chantier séparé plutôt que greffé à la hâte sur du code de paiement qui fonctionne. Texte d'origine : **+40 TND** au parrain (≈ 2× `HOST_VERIFICATION_PRICE_TND`). Dépensable sur vérification Wakil / boost à la une / abonnement — réutilise `VerificationCreditOrder`/`FeaturedOrder`/`Subscription` tels quels, **zéro nouveau rail de paiement**. Plafond : 5 filleuls récompensés par an et par compte. |
| CR3 | Crédit de bienvenue générique (sans parrain) | P2 | ✅ | PR #193. **+10 TND** (`WELCOME_CREDIT_TND`, motif `BIENVENUE_SPONTANE`) accordé dans `verifyPhoneOtpAction` (`src/actions/kyc.ts`) — réutilise le gate KYC téléphone existant, anti-fake-account. `grantWelcomeCreditIfEligible` (`src/lib/credits.ts`) : jamais cumulé avec le crédit filleul (`BIENVENUE_PARRAINAGE`, §CR1) — vérifie qu'aucun des deux motifs de bienvenue n'a déjà été émis avant d'agir (couvre aussi le cas `phoneVerified` qui repasse par false→true après changement de numéro). Best-effort, jamais bloquant. Tests : `tests/credits.test.ts`, `tests/kyc-actions.test.ts`. |
...
| PM2 | Nudge automatique : `ensurePromoSuggestionNotifications` (même idiome que `ensureExpiringSoonNotifications`, **aucun cron**) | P1 | ✅ | PR #199. Déclencheur : annonce `ACTIVE` vérifiée depuis > 14 jours (`PROMO_SUGGESTION_VERIFIED_SINCE_DAYS`) ET zéro résa `CONFIRMEE` chevauchant les 21 prochains jours (`PROMO_SUGGESTION_BOOKING_WINDOW_DAYS`). Message qualitatif (nuits vides + comparaison au prix moyen ville + gain « vendre une nuit de plus », **jamais un total prédit**) — le texte n'est jamais figé en base (convention du fichier), donc pas d'appel direct à `computeYield`/`avgNightCity` dans `notification-center.ts` : la comparaison reste au niveau du message, pas d'un chiffre calculé stocké. Dédup par `href` bucketé par mois (`?promo=AAAA-MM`) via un nouvel index unique partiel (`Notification_promo_suggestion_unique`, migration `20260727072739_add_promo_suggestion_notification`) — au plus un rappel par annonce par mois. Tests : `tests/promo-suggestion-notification.test.ts` + vérification d'intégration réelle contre la base locale. |
...
**Extensions :**
6. ✅ PM2 — nudge automatique (PR #199).
7. 🔧 CR2 — parrainage hôte (détection + crédit faits, PR #192 ; dépense sur vérif/boost/abonnement restante).
