...
|---|---|---|---|---|
| CR0 | Fondations : modèle `CreditWallet` (solde TND, créé paresseusement comme `VerificationWallet`) + `CreditTransaction` (ledger : montant, motif, expiration, résa/filleul liés) + `User.referralCode` (unique) / `User.referredById` (auto-relation, posée une seule fois, jamais modifiable) | P0 | ❌ | Motifs : `BIENVENUE_PARRAINAGE` \| `BIENVENUE_SPONTANE` \| `PARRAINAGE_FILLEUL_TERMINE` \| `UTILISATION_RESERVATION` \| `UTILISATION_SERVICE_HOTE` \| `EXPIRATION` \| `AJUSTEMENT_ADMIN`. Chaque crédit expire 6 mois après émission (levier de rentabilité, cf. CR6). |
| CR1 | Parcours voyageur : page « Mes crédits », bouton « Parrainer » (réutilise `ShareButton.tsx`, F8), application du solde au checkout | P0 | ❌ | Filleul : **+15 TND** immédiat à l'inscription via lien/code. Application plafonnée à 30 % du total ET jamais sous `serviceFee` — le crédit ne réduit que ce que le voyageur paie, jamais la commission garantie ni le prix hôte. |
...
| CR4 | QA/sécurité transverse — **à livrer AVEC chaque phase ci-dessus, pas à la fin** (même règle que MI6/PSP7) | P0 | ❌ | IDOR (impossible de créditer/débiter le wallet d'un autre) ; idempotence de l'émission (un crédit parrain/filleul par filleul, jamais deux, `updateMany`/contrainte unique) ; non-bypass du plancher `serviceFee` à la dépense ; rate-limit génération/consommation de codes de parrainage ; restitution du crédit si la résa associée est annulée-remboursée ; expiration effective (crédit expiré = inutilisable). |
...
5. ❌ CR4 / PM5 — au fur et à mesure de CR0-CR1 et PM0-PM1.
