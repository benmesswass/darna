...
| PSP5 | Dashboard hôte « Factures » (liste, statut, payer) **+ rappels** (J-3, en retard) | P1 | ✅ | `src/app/dashboard/factures/page.tsx` (liste hôte, distincte de la page de paiement unique `[id]` et du dashboard admin §PSP8), `ensureHostInvoiceReminders` (`src/lib/notification-center.ts`, même idiome que `ensureExpiringSoonNotifications`, sondée depuis `src/app/api/notifications/route.ts`), deux nouveaux `NotificationType` (`FACTURE_BIENTOT_DUE`/`FACTURE_EN_RETARD`) chacun avec son propre index unique partiel (migration `20260707170000_add_host_invoice_reminder_notif`), e-mails `sendHostInvoiceDueSoonEmail`/`sendHostInvoiceOverdueEmail` (`src/lib/notifications.ts`), pastille nav `/dashboard/factures` (`src/app/dashboard/layout.tsx`). **Relances désormais automatiques** (`LANCEMENT_ROADMAP.md` §L3.4, `src/lib/jobs/host-invoice-reminder.ts`, tick 15 min) — l'ancienne LIMITE ASSUMÉE (détection paresseuse au seul sondage, hôte connecté) est levée : le job réutilise `ensureHostInvoiceReminders` pour tous les hôtes concernés, connectés ou non ; le sondage au chargement du dashboard reste un filet immédiat en complément. 4 tests (`tests/host-invoice-reminders.test.ts`) + tests dédiés au job. |
